...
const MembersFilter = () => {
  ...
  const selectedIndex = departments.indexOf(selected);
  ...
  const moveLeft = () => {
    ...
    setSelected(departments[prevIndex]);
  };

  const moveRight = () => {
    const nextIndex = (selectedIndex + 1) % departments.length;
    setSelected(departments[nextIndex]);
  };
  ...
  return (
    ...
    <div className="flex flex-col items-center justify-center w-full md:hidden border-b border-gray-400 max-w-[70vw] mt-4 px-10">
        ...
        />
        <div className="px-4 pb-1 py-1.5 text-black text-nowrap min-w-30 flex justify-center">
            {selected}
        </div>
        <FaArrowCircleRight
            onClick={moveRight}
        ...
        <div className="flex justify-between gap-2">
            ...
            <button
                ...
                className={`min-w-20 max-h-10 text-center px-2 pb-1 py-1.5 rounded-t-2xl transition-colors duration-300 ease-in-out text-black font-medium whitespace-nowrap cursor-pointer ${
                    selected === dept ? 'bg-[#BDFF30]' : 'bg-transparent'
                }`}
                >
                {dept}
            </button>
    ...
    {/* DESKTOP */}
    <div className="hidden lg:flex items-center border-b border-gray-400 max-w-[90vw] mt-4 px-10">
        <div className="flex flex-wrap justify-center gap-1 w-full">
        {departments.map((dept) => (
            <button
            key={dept}
            onClick={() => setSelected(dept)}
            className={`px-4 pb-1 py-1.5 rounded-t-2xl transition duration-200 text-black font-medium whitespace-nowrap cursor-pointer ${
                selected === dept ? 'bg-[#BDFF30] text-[#F530FD]' : 'bg-transparent'
            }`}
            >
            {dept}
            </button>
